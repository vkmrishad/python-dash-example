...
graph_page_layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col(dcc.Dropdown(
            id='x-axis-dropdown',
            options=[{'label': col, 'value': col} for col in ['Days to Ship', 'Discount', 'Profit', 'Profit Ratio', 'Quantity', 'Returns', 'Sales']],
    ...
    dbc.Row([
        dbc.Col(dcc.Dropdown(
            ...
            options=[{'label': col, 'value': col} for col in ['Segment', 'Delivery Mode', 'Customer Name', 'Category', 'Sub-Category', 'Product Name']],
...
def register_callbacks(app):
    @app.callback(
        ...
        [Input('date-picker-range', 'start_date'), Input('date-picker-range', 'end_date'), Input('granularity-dropdown', 'value')]
    )
    ...
    @app.callback(
        ...
        [Input('x-axis-dropdown', 'value'), Input('y-axis-dropdown', 'value'), Input('size-dropdown', 'value'), Input('breakdown-dropdown', 'value')]
    )
    def update_bubble_chart(x_axis, y_axis, size, breakdown):
        if x_axis and y_axis and size and breakdown:
            fig = px.scatter(df, x=x_axis, y=y_axis, size=size, color=breakdown, title=f'{y_axis} vs {x_axis}')
            return fig
        return {}
